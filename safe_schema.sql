...
-- Configurar RLS (Policies) para os buckets criados

-- 1. Leitura pública (qualquer um pode ver as imagens)
CREATE POLICY "Public Access" 
ON storage.objects FOR SELECT 
USING ( bucket_id IN ('event-flyers', 'logos') );

-- 2. Escrita apenas para usuários autenticados
CREATE POLICY "Auth Insert" 
ON storage.objects FOR INSERT 
TO authenticated 
WITH CHECK ( bucket_id IN ('event-flyers', 'logos') );
...
-- 4. Exclusão apenas para usuários autenticados
